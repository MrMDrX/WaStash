...
import 'package:wastash/widgets/options_modal.dart';
...
class _StatusPageState extends State<StatusPage>
            ...
            SliverToBoxAdapter(
              child: Padding(
                ...
                child: Column(
                  children: [
                    Text(
                      context.l10n.statusSaver,
                      style: context.textTheme.titleLarge,
                    ),
                    const SizedBox(height: 8),
                    Text(
                      context.l10n.statusSaverSubtitle,
          ...
          body: _selectedDirectory == null
              ? Center(
                  child: Column(
                    ...
                    children: [
                      Text(
                        context.l10n.storageAccessIsRequired,
                        textAlign: TextAlign.center,
                        style: TextStyle(
                          fontSize: 16,
                      ...
                      ElevatedButton(
                        ...
                        child: Text(context.l10n.selectStatusFolder),
                      ),
                    ],
                  ),
                )
  ...
  Widget _buildContent(StatusState state, bool isDark) {
    if (state is StatusLoading) {
      return _buildShimmerLoading(isDark);
    } else if (state is StatusError) {
      return Center(child: Text('Error: ${state.message}'));
    } else if (state is StatusLoaded) {
      _images = state.files.where(StatusRepository.isImage).toList();
      _videos = state.files.where(StatusRepository.isVideo).toList();
      return Column(
        children: [
          Container(
            height: 48,
          ...
          Expanded(
            child: TabBarView(
              controller: _tabController,
              children: [_buildMediaGrid(_images), _buildMediaGrid(_videos)],
            ),
          ),
        ],
      );
  ...
  Widget _buildMediaGrid(List<String> uris) {
    if (uris.isEmpty) {
      return Center(child: Text(context.l10n.noStatus));
    }
  ...
  void _requestAccess() async {
    showModalBottomSheet(
      ...
      builder: (context) => OptionsModal(
        ...
        message: context.l10n.accessHowTo,
        options: [
          OptionItem(
